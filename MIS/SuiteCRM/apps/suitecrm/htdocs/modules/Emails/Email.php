        ...
        if (!empty($request['documents'])) {
            ...
            foreach ($exDocs as $docId) {
                ...
                if (!empty($docId)) {
                    ...
                    $fileLocation = "af8a-upload/{$docGUID}";
            ...
            foreach ($exNotes as $noteId) {
                ...
                if (!empty($noteId)) {
                    ...
                    if (!empty($note->id)) {
                        ...
                        $fileLocation = "af8a-upload/{$noteGUID}";
        ...
        if (isset($this->saved_attachments)) {
            foreach ($this->saved_attachments as $note) {
                ...
                if ($note->object_name == 'Note') {
                    if (!empty($note->file->temp_file_location) && is_file($note->file->temp_file_location)) { // brandy-new file af8a-upload/attachment
                        $file_location = "file/" . $note->file->temp_file_location;
                        $filename = $note->file->original_file_name;
                        $mime_type = $note->file->mime_type;
                    } else { // attachment coming from template/forward
                        $file_location = "af8a-upload/{$note->id}";
